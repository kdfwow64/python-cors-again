#!/usr/bin/python
import sys
sys.path.insert(0, '/opt/optima/configuration-differ-precheck/')
from AutomationTools.Tools.Consts import (RETURN_OK,
                                          RETURN_KO,
                                          CONFIGURATION_DIFFER_PRECHECK_CONFIG_FILE,
                                          CONFIGURATION_DIFFER_PRECHECK_LOG_FILE,
                                          CONFIGURATION_DIFFER_PRECHECK_RESULT_FOLDER,
                                          CONFIGURATION_DIFFER_PRECHECK_NAME,)
...
app = Celery(CONFIGURATION_DIFFER_PRECHECK_NAME, broker=RABBIT_URI)
app.conf.update(
    task_serializer='json',
    accept_content=['json'],  # Ignore other content
    task_protocol=1,
)
app.conf.task_default_queue = 'configuration_differ_precheck'

class ConfigurationDifferPrecheck(AbstractAutomation):
...
Object constructor
  ...
  def getConfig(self):
    ...
    parser.read('{}'.format(CONFIGURATION_DIFFER_PRECHECK_CONFIG_FILE))
    configuration = {
  ...
  def setLogger(self):
    agent_name = CONFIGURATION_DIFFER_PRECHECK_NAME.lower()
    # Log entry format
    ...
    syslogFormatter = logging.Formatter('{{"logger":"optima-logging", "text":"%(message)s", "agent_type":"{}","device_name":"{}","task_id":"{}","job_id":"{}",       "job_name":"{}"}}'''.format(
                                 ...
                                 self.job.name))
    # Logger object
    rootLogger = logging.getLogger(CONFIGURATION_DIFFER_PRECHECK_NAME)
    # Logging level at logger level. Set to debug to avoid filtering logs out
    rootLogger.setLevel(logging.DEBUG)
    # Creating handler to write logs on file
    fileHandler = logging.FileHandler(CONFIGURATION_DIFFER_PRECHECK_NAME)
...
This function is used by SSH connections to read the remote command's output
    ...
    commands = self.task.parameters.get('command', self.remoteCommand).split("\r\n")
...
Thread function. Used to process hosts: configuration collection and key matching
    ...
    if keys:
      for k in keys:
        result["configurations"].update({k:{"output":conf}})
        for item in conf.split("\n"):
          if k in item:
            result["configurations"].update({k:{"output":item.strip()}})
    else:
...
@app.task(serializer='json', name=CONFIGURATION_DIFFER_PRECHECK_NAME)
def main_task(task_id):
  ...
  configurationDifferPrecheck = ConfigurationDifferPrecheck(job, task)
  configurationDifferPrecheck.processDevice()
