#!/usr/bin/python
import sys
sys.path.insert(0, '/opt/optima/configuration-parser/')

from AutomationTools.Tools.Consts import RETURN_OK, RETURN_KO, CONFIGURATION_PARSER_NAME, CONFIGURATION_PARSER_CONFIG_FILE, CONFIGURATION_PARSER_LOG_FILE, CONFIGURATION_PARSER_RESULT_FOLDER
...
app = Celery(CONFIGURATION_PARSER_NAME, broker=RABBIT_URI)
app.conf.update(
    task_serializer='json',
    accept_content=['json'],  # Ignore other content
    task_protocol=1,
)
app.conf.task_default_queue = 'configuration_parser'

class ConfigurationParser(AbstractAutomation):
...
Object constructor
    ...
    self.keyList = job.parameters.get('keyList', [])
    self.strictMatchingFlag = job.parameters.get('strictMatching')
  ...
  def getConfig(self):
    ...
    parser.read(CONFIGURATION_PARSER_CONFIG_FILE)
    configuration = {
  ...
  def setLogger(self):
    agent_name = CONFIGURATION_PARSER_NAME.lower()
    # Log entry format
    ...
    syslogFormatter = logging.Formatter('{{"logger":"optima-logging", "text":"%(message)s", "agent_type":"{}", "device_name":"{}", "task_id":"{}", "job_id":"{}", "job_name":"{}"}}'''.format(
                                 ...
                                 self.job.name))
    # Logger object
    rootLogger = logging.getLogger(CONFIGURATION_PARSER_NAME)
    # Logging level at logger level. Set to debug to avoid filtering logs out
    rootLogger.setLevel(logging.DEBUG)
    # Creating handler to write logs on file
    fileHandler = logging.FileHandler(CONFIGURATION_PARSER_LOG_FILE)
  ...
  def sendToQueue2(self, producer, task, data):
    errors = 0
    while errors <= 5:
      try: 
        ...
        self.rootLogger.exception('Could not insert to queue')
        errors += 1
...
Thread function. Used to process hosts: configuration collection and key matching
    ...
    keys = self.keyList
    for k in keys:
      result["configurations"].update({k:{"output":""}})
      lines = result["configurations"][k]["output"]
      for item in conf.split("\n"):
        if self.strictMatchingFlag:
          strict_match = re.compile(k)
          match = re.findall(strict_match,item)
          if match:
            lines = lines + "\r\n" + item.strip()
        else:
          if k in item:
      ...
      if lines.startswith("\n"):
        lines = lines[1:]
      result["configurations"][k]["output"] = lines
...
@app.task(serializer='json', name=CONFIGURATION_PARSER_NAME)
def main_task(task_id):
  ...
  configurationParser = ConfigurationParser(job, task)
  configurationParser.processDevice()
