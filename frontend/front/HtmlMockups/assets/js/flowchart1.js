...
// "colorize" the widget name
    $.widget("flowchart1.flowchart1", {
        ...
        options: {
            ...
            data: {},
            distanceFromArrow: 3,
            defaultOperatorClass: 'flowchart1-default-operator',
        ...
        // the constructor
        _create: function () {
            if (typeof document.__flowchart1Number == 'undefined') {
                document.__flowchart1Number = 0;
            } else {
                document.__flowchart1Number++;
            }
            this.globalId = document.__flowchart1Number;
            this._unitVariables();

            this.element.addClass('flowchart1-container');

            this.objs.layers.links = $('<svg class="flowchart1-links-layer"></svg>');
            this.objs.layers.links.appendTo(this.element);

            this.objs.layers.operators = $('<div class="flowchart1-operators-layer unselectable"></div>');
            this.objs.layers.operators.appendTo(this.element);

            this.objs.layers.temporaryLink = $('<svg class="flowchart1-temporary-link-layer"></svg>');
        ...
        _initEvents: function () {
            ...
            this.objs.layers.operators.on('pointerdown mousedown touchstart', '.flowchart1-operator', function (e) {
                e.stopImmediatePropagation();
            });

            this.objs.layers.operators.on('click', '.flowchart1-operator', function (e) {
                if ($(e.target).closest('.flowchart1-operator-connector').length == 0) {
                    self.selectOperator($(this).data('operator_id'));
                }
            });

            this.objs.layers.operators.on('click', '.flowchart1-operator-connector', function () {
                var $this = $(this);
                if (self.options.canUserEditLinks) {
                    self._connectorClicked($this.closest('.flowchart1-operator').data('operator_id'), $this.data('connector'), $this.data('sub_connector'), $this.closest('.flowchart1-operator-connector-set').data('connector_type'));
                }
            });

            this.objs.layers.links.on('mousedown touchstart', '.flowchart1-link', function (e) {
                e.stopImmediatePropagation();
            });

            this.objs.layers.links.on('mouseover', '.flowchart1-link', function () {
                self._connecterMouseOver($(this).data('link_id'));
            });

            this.objs.layers.links.on('mouseout', '.flowchart1-link', function () {
                self._connecterMouseOut($(this).data('link_id'));
            });

            this.objs.layers.links.on('click', '.flowchart1-link', function () {
                self.selectLink($(this).data('link_id'));
            });

            this.objs.layers.operators.on('mouseover', '.flowchart1-operator', function (e) {
                self._operatorMouseOver($(this).data('operator_id'));
            });

            this.objs.layers.operators.on('mouseout', '.flowchart1-operator', function (e) {
        ...
        _clearLinksLayer: function () {
            ...
            this.objs.layers.operators.find('.flowchart1-operator-connector-small-arrow').css('border-left-color', 'transparent');
        },
        ...
        _drawLink: function (linkId) {
            ...
            group.setAttribute('class', 'flowchart1-link');
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var $operator = $('<div class="flowchart1-operator"></div>');
            $operator.addClass(infos.class);

            var $operator_title = $('<div class="flowchart1-operator-title"></div>');
            $operator_title.html(infos.title);
            $operator_title.appendTo($operator);

            var $operator_inputs_outputs = $('<div class="flowchart1-operator-inputs-outputs"></div>');

            $operator_inputs_outputs.appendTo($operator);

            var $operator_inputs = $('<div class="flowchart1-operator-inputs"></div>');
            $operator_inputs.appendTo($operator_inputs_outputs);

            var $operator_outputs = $('<div class="flowchart1-operator-outputs"></div>');
            ...
            function addConnector(connectorKey, connectorInfos, $operator_container, connectorType) {
                var $operator_connector_set = $('<div class="flowchart1-operator-connector-set"></div>');
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            var $operator_connector = $('<div class="flowchart1-operator-connector"></div>');
            ...
            var $operator_connector_label = $('<div class="flowchart1-operator-connector-label"></div>');
            ...
            var $operator_connector_arrow = $('<div class="flowchart1-operator-connector-arrow"></div>');

            $operator_connector_arrow.appendTo($operator_connector);

            var $operator_connector_small_arrow = $('<div class="flowchart1-operator-connector-small-arrow"></div>');
        ...
        createOperator: function (operatorId, operatorData) {
            ...
            if (this.options.canUserMoveOperators) {
                ...
                fullElement.operator.draggable({
                    ...
                    handle: '.flowchart1-operator-title',
                    start: function (e, ui) {
        ...
        _click: function (x, y, e) {
            var $target = $(e.target);
            if ($target.closest('.flowchart1-operator-connector').length == 0) {
                this._unsetTemporaryLink();
            }

            if ($target.closest('.flowchart1-operator').length == 0) {
                this.unselectOperator();
            }

            if ($target.closest('.flowchart1-link').length == 0) {
                this.unselectLink();
            }
        },

        _removeSelectedClassOperators: function () {
            this.objs.layers.operators.find('.flowchart1-operator').removeClass('selected');
        },
        ...
        removeClassOperators: function (className) {
            this.objs.layers.operators.find('.flowchart1-operator').removeClass(className);
        },
        ...
        _removeHoverClassOperators: function () {
            this.objs.layers.operators.find('.flowchart1-operator').removeClass('hover');
        },
